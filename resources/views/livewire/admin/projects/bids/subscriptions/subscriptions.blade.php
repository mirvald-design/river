    ...
    <div class="py-4 px-5 lg:px-6 w-full sm:flex sm:justify-between sm:items-center">
        <div class="flex justify-center sm:justify-left">
            <h3 class="inline-flex items-center font-semibold">
                <span>@lang('messages.t_bids_subscriptions')</span>
            </h3>
        </div>
    </div>
                    ...
                    @foreach ($subscriptions as $s)
                        <tr class="hover:bg-gray-50" wire:key="subscriptions-{{ str_replace('-', '_', $s->uid) }}">
                            ...
                            <td class="px-7 py-4 ltr:text-left rtl:text-right max-w-xs">
                                <a href="{{ url('profile', $s->bid->user->username) }}" target="_blank" class="flex items-center">
                                    <div class="w-8 h-8">
                                        <img class="w-full h-full rounded object-cover lazy" src="{{ placeholder_img() }}" data-src="{{ src($s->bid->user->avatar) }}" alt="{{ $s->bid->user->username }}" />
                                    </div>
                                    <div class="ltr:pl-4 rtl:pr-4">
                                        <p class="font-medium text-xs flex items-center">
                                            {{ $s->bid->user->username }}
                                            @if ($s->bid->user->status === 'verified')
                                                <img data-tooltip-target="tooltip-account-verified-{{ $s->bid->user->id }}" class="ltr:ml-0.5 rtl:mr-0.5 h-3.5 w-3.5 -mt-0.5" src="{{ url('public/img/auth/verified-badge.svg') }}" alt="{{ __('messages.t_account_verified') }}">
                                                <div id="tooltip-account-verified-{{ $s->bid->user->id }}" role="tooltip" class="inline-block absolute invisible z-10 py-2 px-3 text-sm font-medium text-white bg-gray-900 rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                                    {{ __('messages.t_account_verified') }}
                                                </div>
                                            @endif
                                        </p>
                                        <p class="text-xs leading-3 text-gray-600 pt-2">{{ $s->bid->user->email }}</p>
                                    </div>
                                </a>
                            </td>
